import Validator from 'ember-attribute-validations/validators/max';
...
test('validate string', function(assert) {
  ...
  assert.deepEqual(validator.validate('email', undefined, attribute, {}), undefined);
  assert.deepEqual(validator.validate('email', '', attribute, {}), undefined);

  assert.deepEqual(validator.validate('email', false, attribute, {}), 'Email must not be longer than 10');
  assert.deepEqual(validator.validate('email', 'some email value', attribute, {}), 'Email must not be longer than 10');
});
...
test('validate number', function(assert) {
  ...
  assert.deepEqual(validator.validate('rating', false, attribute, {}), 'Rating must not be bigger than 5');
  assert.deepEqual(validator.validate('rating', 6, attribute, {}), 'Rating must not be bigger than 5');
  assert.deepEqual(validator.validate('rating', 8, attribute, {}), 'Rating must not be bigger than 5');
});
