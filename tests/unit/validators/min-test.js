...
test("validate string", function (assert) {
  ...
  assert.deepEqual(
    validator.validate("email", "testing value", attribute, {}),
    undefined
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    ...
    "Email must not be shorter than 3"
  );
  assert.deepEqual(
...
test("validate number", function (assert) {
  ...
  assert.deepEqual(
    validator.validate("rating", 1, attribute, {}),
    "Rating must not be lesser than 5"
  );
  assert.deepEqual(
    validator.validate("rating", null, attribute, {}),
    "Rating must not be lesser than 5"
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    validator.validate("rating", undefined, attribute, {}),
    "Rating must not be lesser than 5"
  );
});
